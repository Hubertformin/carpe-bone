    ...
    this.start = function () {
        webSocket.createServer(function (conn) {
            ...
            function sendAnyChanges() {
                ...
                const changes = db.changes.filter(function (change) { return change.rev > syncedRevision && change.source !== conn.clientIdentity; });
                ...
                if (type == "clientIdentity") {
                    ...
                    try {
                        ...
                        if (request.partial) {
                            ...
                            const serverChanges = db.changes.filter(function (change) { return change.rev > baseRevision });
